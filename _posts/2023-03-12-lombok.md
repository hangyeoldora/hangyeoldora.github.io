...
​	lombok은 다양한 ide와 빌드 툴에 대해 제공을 한다. 이클립스와 스프링 툴 슈트를 이용할 때는 <a href="https://projectlombok.org/download" target="_blank">다운로드</a> 링크에서 lombok.jar를 다운을 받고 실행하면 된다. 

![lombok 설치](/assets/img/lombok_install.png)

​	실행을 하게 되면 위와 같은 화면이 나오게 된다. 자동으로 ide를 잡아주는데, 없을 경우에는 _Specify location_을 누르면 되고, 정상적일 경우에는 진행을 하면 된다.
...
​	lombok을 사용할 준비가 되었다면, 상용구 코드를 작성하면 되는데 ExVo를 통해 기존 코드와 lombok을 활용한 코드에 대해 알아보겠다.
...
​	lombok 어노테이션으로 기존과 동일하게 주입이 가능하고 값을 받을 수 있으며, <u>maven package</u>를 통한 빌드도 성공적으로 수행이 가능하다.
