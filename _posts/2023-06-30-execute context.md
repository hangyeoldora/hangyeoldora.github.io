...
8. 원시값에 프로퍼티 설정 금지
   ...
   - 기본 모드가 아닌 엄격 모드에서는 TypeError를 발생시킨다. 
...
1. this로 함수에 전달된 값은 객체가 되지 않음
   ...
   - 일반적인 함수에서 this는 언제나 객체인데, 이러한 자동 박싱은 성능 비용뿐만 아니라 전역 객체가 노출되어 위험하다.
     ...
     console.assert(test() === undefined); // undefined
     console.assert(test.call(2) === 2); // 2
     console.assert(test.apply(null) === null) // undefined
     console.assert(test.bind(true)() === true) // true
     ```

   - 특정 this를 명세하기 위해서는 .call, .apply, .bind 메서드를 통해 가능


     -> 이는 더 이상 window 객체를 this로 참조가 불가능하다는 것을 의미
...
- 실행 컨텍스트
  ...
  - 특정 자바스크립트를 실행하는데 필요한 환경 정보를 모아둔 객체
  - 자바스크립트 코드는 항상 실행 컨텍스트 내에서 실행 중 

  - Variable Environment, Laxical Environment는 실행 컨텍스트 내에서 변수의 참조들을 기억하는 단계
